...
用法：
1. 实现WhereGroups接口，用于定义查询条件（代码生成插件自动生成）。
```java
...
2. 通过建立`NebulaSQL`实例，来创建SQL语句对象。
```java
...
3. 通过实现Mybatis的`BaseMapper`接口，来实现自定义的SQL语句。
```java
...
4. 通过代码生成插件，生成对应的`XXXMapper.xml`通用文件，来实现方法的调用和实现。
